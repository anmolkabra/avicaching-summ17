\begin{appendices}
    ...
    As the initial approach, we logged details of main memory usage when the models were running. Contrary to our expectations (again), the GPU ``set'' was utilizing more main memory than CPU ``set'' was. Precisely, when running the Pricing Problem's model with $T = 173, J = 116$ and logging 5 times at 0.5 second intervals, the GPU ``set'' was using 9.1 \% of the main memory, whereas CPU ``set'' was using only 0.9 \% memory. Although we 
\end{appendices}