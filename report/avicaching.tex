...
%    \end{tikzpicture}
    ...
    \begin{algorithm}
        ...
        \begin{algorithmic}[1]
            \State $\mathbf{d} \gets \Call{Normalize}{\mathbf{d}}$\Comment{$\mathbf{d}[u][v]$ is the distance between locations $u$ and $v$}
            \State $\mathbf{f} \gets \Call{Normalize}{\mathbf{f}, axis = 0}$\Comment{$\mathbf{f}[u]$ is a vector of env. features at location $u$}
            \State $\mathbf{r_t} \gets \Call{Normalize}{\mathbf{r_t}, axis = 0}$\Comment{$\mathbf{r_t}[u]$ is the reward at location $u$}
            \For{$v = 1, 2, \dots J$}
                \For{$u = 1, 2, \dots J$}
