...
\begin{document}
    ...
    Taking the approach mentioned in Section \ref{sec:Pricing Problem-Optimizing the Original Dataset}, different sets of weights gave consistent loss values (even with differently seeded rewards), which are shown in Figure []. Next, running differently seeded rewards with different learning rates on set-2 of weights, which performed the best, we obtained the lowest loss value of 0.0068\%.
    \begin{figure}[!htbp]
        \centering
        \begin{tikzpicture}
        \begin{axis}[
