...
\begin{document}
    ...
    % terms
    \nomenclature[T]{CPU}{Central Processing Unit: 1-8 cores with higher clockspeed and wider instruction set per core}
    \nomenclature[T]{GPU}{Graphical Processing Unit: >1000 cores with lower clockspeed and smaller instruction set per core}
    ...
    \begin{wrapfigure}{r}{.4\textwidth}
        ...
        \caption[Predicted Probabilities of Agents Visiting Each Location Plotted on a Map]{Predicted Probabilities of Agents Visiting Each Location Plotted on a Map: Dark dots represent high prediction of visits. This can be compared to the map plots for the 2-layered network and other procedures\cite[Figure 3]{Xue2016Avi2}.}
    ...
    \begin{figure}[h]
        ...
        \begin{subfigure}{.49\textwidth}
            ...
            ] coordinates {(11,11.37) (55,123.04) (116,257.79)};  % cpu
            \addplot+ [mark=*,] coordinates {(11,6.72) (55,46.12) (116,154.85)};  % gpu
            \legend{CPU ``set'',GPU ``set''}
            \end{axis}
            \end{tikzpicture}
            \caption{Time taken by the LP}
            \label{Time taken by the LP}
        \end{subfigure}
        \begin{subfigure}{.49\textwidth}
            \centering
            \begin{tikzpicture}
            ...
            \addplot+ [mark=*,] coordinates {(11,6.01) (55,9.77) (116,16.30)};  % cpu
            \addplot+ [mark=*,] coordinates {(11,1.11) (55,1.44) (116,1.94)};  % gpu
            \legend{CPU ``set'',GPU ``set''}
            \end{axis}
            \end{tikzpicture}
            \caption{Time taken by the Neural Network}
            \label{Time taken by the Neural Network}
        \end{subfigure}
    ...
    Furthermore, though we expected SciPy's Optimize Module to run on the CPU (Section \ref{sec:Pricing Problem-Testing GPU Speedup on the Random Dataset}), the Linear Programming part ran faster when the GPU was ``set''. We did not anticipate this behavior, and we couldn't pick out a reason for the Optimize Module supposedly doing operations differently. Since our GPU ``set'' and CPU ``set'' configurations were only enforceable on user-created functions and datasets, we do not know why the LP solver delivered different speeds while solving the same problem. If it used the CPU or the GPU, it should have done so in both cases and the runtimes would have been equal. While there could be differences in implementation, we cannot affirm or deny any reason behind this quirky behavior. Figure [] shows the unexpected speedup delivered by SciPy's Optimize Module.
    
    Nevertheless, we know that the LP problem slowed the overall model down considerably
