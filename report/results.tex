...
After running on different batch-sizes $J = \{11, 37, 63, 90, 116, 145, 174, 203, 232\}$, we \textbf{did not observe drastic GPU speedup} for the full model. \Cref{fig:Finding Rewards - Time Taken by the Full Model} shows the Speedup trend: GPU Speedup for the full model was a mere \textbf{1.34 $\pm$ 0.16}.
\begin{figure}[!htbp]
    \centering
    \begin{subfigure}{\textwidth}
        ...
        \begin{tikzpicture}
            ...
            \end{axis}
        \end{tikzpicture}
        \caption{Time Taken by the Full Model: GPU Speedup over all batch-sizes is only \textbf{1.34 $\pm$ 0.16}.}
    ...
    \begin{subfigure}{\textwidth}
        ...
        \begin{tikzpicture}
            ...
            \end{axis}
        \end{tikzpicture}
        \caption[Time taken by the LP]{Time taken by the LP: GPU Speedup over all batch-sizes is only \textbf{1.37 $\pm$ 0.16}. As discussed in \Cref{sec:PriProbRes - GPU,app:GPU Speedup in LP Computation}, we shouldn't have witnessed this speedup as both configurations' LPs were computed on the CPU. Hence, we expected the GPU Speedup value for LP to be $\approx$ 1. }
...
\begin{figure}[!htbp]
    ...
    \begin{subfigure}{\textwidth}
        ...
        \end{axis}
        \end{tikzpicture}
        \caption{Time Taken by the Full Model: GPU Speedup over all batch-sizes is only \textbf{0.98 $\pm$ 0.04} -- heavily impacted by LP runtimes.}
    ...
    \begin{subfigure}{\textwidth}
        ...
        \end{axis}
        \end{tikzpicture}
        \caption[Time taken by the LP]{Time taken by the LP: GPU Speedup over all batch-sizes is only \textbf{1.002 $\pm$ 0.005}. Expectedly, there is no GPU Speedup in LP runtimes.}
...
\begin{figure}[!htbp]
    ...
    \begin{subfigure}{\textwidth}
        ...
        \end{axis}
        \end{tikzpicture}
        \caption{Time taken by the Neural Network: GPU Speedup over all batch-sizes is \textbf{1.41 $\pm$ 0.75} -- as transfer time between RAM and GPU's internal memory dominates for smaller datasets. However, as batch-size increases, computation time dominates over transfer time -- GPU ``set'' performs better than CPU ``set''.}
